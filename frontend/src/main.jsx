...
function MusicPage() {
  ...
  async function runMusicSearch(term, nextPage = 1, append = false, sourceOverride = '') {
    ...
    setSearching(true); setErr(''); setPopoverOpen(true)
    try {
      const src = sourceOverride || sourceId || info?.config?.default_source || ''
      const res = await fetch(`${API_BASE}/api/music/search?q=${encodeURIComponent(q)}&source=${encodeURIComponent(src)}&page=${nextPage}&limit=10`)
      const json = await res.json().catch(() => ({}))
  ...
  function onTimeUpdate() {
    ...
    setCurrentTime(t); setDuration(Number.isFinite(a.duration) ? a.duration : 0)
    let idx = -1
  ...
  return <><PageChrome /><div className="main-content market-page">
    <section className="market-asset-bar card animate-fadeInUp">
      <div className="market-balance-block">
        <span className="channel-pill">泓市场</span>
        <h1>{Number(data?.balance || 0).toLocaleString()} <small>泓币</small></h1>
        <p>{me ? '社区贡献会沉淀成泓币，可兑换权益与实物。' : '访客可浏览商品；登录后可签到、兑换和查看记录。'}</p>
      </div>
      <div className="market-actions-block">
        {me ? <button className="btn btn-primary market-checkin-btn" onClick={checkin}><i className="fas fa-calendar-check" /> 每日签到</button> : <a className="btn btn-primary market-checkin-btn" href="/login"><i className="fas fa-right-to-bracket" /> 登录后兑换</a>}
        <div className="market-rules">{(data?.rules || []).map(x => <span key={x}>{x}</span>)}</div>
      </div>
    </section>
    {err && <div className="alert alert-error">{err}</div>}
    ...
    <div className="card animate-fadeInUp market-orders-card"><div className="card-header fold-head"><h3><i className="fas fa-receipt" /> {me ? '我的兑换记录' : '兑换记录'}</h3><button className="btn btn-sm btn-secondary" onClick={() => setShowOrders(!showOrders)}><i className={`fas ${showOrders ? 'fa-chevron-up' : 'fa-chevron-down'}`} /> {showOrders ? '折叠' : '展开'}</button></div>{showOrders && <div>{data?.orders?.length ? data.orders.map(o => <OrderRow order={o} key={o.id} />) : <div className="empty-state"><i className="fas fa-receipt" /><p>{me ? '暂无兑换记录' : '登录后查看个人兑换记录'}</p></div>}</div>}</div>
  </div><ExchangeModal key={activeItem?.id || 'closed'} item={activeItem} balance={data?.balance || 0} busy={Boolean(busy)} onClose={() => setActiveItem(null)} onSubmit={async (item, value) => { await buy(item, value); if (!(item.title === '改名卡' || item.title === '头衔申请券' || item.title === '自定义头衔卡' || isInstantDressupItem(item))) setActiveItem(null) }} /></>
}
...
function AdminArticles({ data, draft, setDraft, run }) {
  ...
  return <div className="admin-card article-admin"><div className="admin-card-head"><h3><i className="fas fa-book-open" /> 文章管理</h3><div className="admin-actions"><span className="storage-pill">社区本地图床</span><span className="storage-pill muted">第三方图床预留</span></div></div>
    <div className="article-admin-grid">
      ...
      <aside className="material-library"><div className="material-head"><h3><i className="fas fa-images" /> 素材库</h3><small>{assets.length}/{assetMeta.total || assets.length}</small></div><div className="form-hint">点击图片查看大图；按钮只复制 Markdown，不会自动改正文。</div><div className="material-grid">{assets.map(a => <div className="material-card" key={a.id}><button type="button" className="material-thumb-btn" onClick={() => setPreviewAsset(a)} title="查看图片"><img src={a.url} alt="" loading="lazy" /></button><small>{a.width}×{a.height}</small><div className="material-actions"><button className="btn btn-sm btn-secondary" onClick={() => copyAsset(a)}>复制 Markdown</button><button className="btn btn-sm btn-secondary" onClick={() => copyText(a.url, '图片链接已复制')}>复制链接</button><button className="btn btn-sm btn-danger" onClick={() => deleteAsset(a)}>删除</button></div></div>)}{!assets.length && <div className="empty-state material-empty"><i className="fas fa-image" /><p>暂无素材</p></div>}</div>{assetMeta.hasMore && <button className="btn btn-secondary material-load-more" disabled={assetMeta.loading} onClick={loadMoreAssets}>{assetMeta.loading ? '加载中...' : '加载更多图片'}</button>}</aside>
    </div>
...
function App() {
  ...
  return <>{isAuth ? page : <div className="app-shell"><Nav me={me} setMe={setMe} path={path} site={site} />{page}<SiteFooter site={site} /></div>}<ToastHost /></>
}
